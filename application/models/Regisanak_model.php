...
class Regisanak_model extends CI_Model
{
    ...
    public function getDataGraph($jg, $bulan = null)
    {
        ...
        if (!is_null($bulan)) {
            if ($jg == 1) {
                $query = "SELECT count(no_pasien) as jml FROM `regisanak` WHERE substr(tgl_daftar,1,7) = substr('$bulan',1,7) GROUP by id_wilayah";
            } else {
                $query = "SELECT count(id_pencatatan) as jml FROM `pencatatan`WHERE substr(tgl_pencatatan,1,7) = substr('$bulan',1,7) GROUP by idWilayah";
            }
        } else {
            if ($jg == 1) {
                $query = "SELECT count(no_pasien) as jml FROM `regisanak` GROUP by id_wilayah";
            } else {
                $query = "SELECT count(id_pencatatan) as jml FROM `pencatatan` GROUP by idWilayah";
            }
        }
